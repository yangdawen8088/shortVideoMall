<template>
	<view class="addform bg-gradual-pink" style="position: fixed;width: 100%;height: 100%;" v-bind:style="{paddingTop:system.statusBarHeight+'px'}">
		...
		<scroll-view scroll-y="true" style="width: 100%;height: 100%;padding-top: 100upx;">
			<view class="cu-list menu sm-border solid-bottom text-white" style="border-color: white;margin-top: 0;" >
				...
				<view class="cu-item arrow" @tap="toTaobao">
					<view class="content">
						<text class="text-white">查看淘宝详情</text>
					</view>
					<view class="action">
						<text class="text-white text-sm">
...
<script>
	export default {
							...
							success:(res) => {
								if("ok" == res.data.status){
									...
									//if(this.categoryCode.length > 0 ){
										...
										if(!find){
											let pos = this.goods.third_url.indexOf("?id=");
											let thirdid = null;
											if(pos >= 0){
												thirdid = this.goods.third_url.substring(pos+"?id=".length, this.goods.third_url.indexOf("&", pos));
											}else if((pos = this.goods.third_url.indexOf("&id=")) >= 0){
												thirdid = this.goods.third_url.substring(pos+"&id=".length, this.goods.third_url.indexOf("&", pos+"&id=".length));
											}
											if(thirdid){
												this.postJson({
													path:'/goods/category/locate',
													data:{
														sid: thirdid
													},
													success:(res_category)=>{
														if("ok" == res_category.data.status && res_category.data.data.length>0){
															let data = res_category.data.data[0];
															let sid = data.sid;
															let sql = "select * from video.goods_category where upbcat='"+sid+"'";
															this.postJson({
																path:'/db.jsp',
																data:{
																	sql: sql
																...
																success:(res_db) => {
																	if("ok" == res_db.data.status && res_db.data.data.length > 0){
																		let goodscategory = res_db.data.data[0];
																		this.categoryCode = goodscategory.category;
																		find = false;
																		for(let x in this.categoryData){
																			let category = this.categoryData[x];
																			for(let y in category.child){
		...
		methods:{
			toTaobao(e){
				uni.navigateTo({
					url:"/pages/download/webpage?url="+encodeURIComponent(this.goods.third_url)
				});
			},
			addStyle(e){
			...
			submit(){
				...
				data.locationObject = this.location;
...
<style>
.addform .cu-list.menu>.cu-item{
	background-color: rgba(0,0,0,0);
}
.addform .cu-item input{
